...
import { TRANSITION_TIMEOUT, THRESHOLD, LOW_GADGET_HEIGHT } from "@/lib/constants";
...
async function makePanelMedium() {
  ...
  await setHeightSmoothly((document.body.clientHeight - MIN_PANEL_HEIGHT) / 2);
}
...
function onDragEnd() {
  ...
  if (panelHeight.value === MIN_PANEL_HEIGHT && dragTargetElement.value?.tagName.toLowerCase() === "input") {
...
<template>
  <div class="app" v-if="appStore.state.center" @touchmove.prevent="onDrag($event.changedTouches[0].clientY)" @mousemove.prevent="onDrag($event.clientY)">
    ...
    <div class="add-object-container">
      <button class="add-object-button" @click="startSelectingLocation">Добавить объект</button>
    </div>
    ...
    <div
      class="app__panel"
      :class="{ 'app__panel--animated': animating, 'app__panel--dragging': dragging, 'app__panel--overlayed': overlayed }"
      ref="panelElement"
      @touchstart="onDragStart($event.changedTouches[0], $event.changedTouches[0].clientY)"
      @touchend="onDragEnd"
    ...
    >
      ...
      <BaseField class="app__search" v-model="query" @update:model-value="onSearch" type="search" placeholder="Поиск мест, скидок, карт поблизости" />

      <TheCatalog :places="appStore.state.places" :renew="renewPlaces" @places:filter="filteredPlaces = $event" @scroll:top="makePanelLow" @scroll:bottom="makePanelHigh" @update:resetRenew="renewPlaces = false" />
    </div>
  </div>
</template>
